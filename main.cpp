...
static int parseCmdArgs(int argc, char** argv)
{
    ...
    for (int i = 1; i < argc; ++i)
    {
        ...
        else if (string(argv[i]) == "--try_gpu")
        {
            ...
            {
                cout << "--try_gpu flag can be either true or false\n";
                return -1;
            }
            i++;
        }
...
int main(int argc, char* argv[])
{
    ...
    if (num_images < 2)
    {
        LOGLN("Stitching must be done with more than 2 images");
        return -1;
    }
    ...
    if (featuresType == "surf")
    {
        ...
        bool ifGpu=gpu::getCudaEnabledDeviceCount();
    ...
    BestOf2NearestMatcher matcher(try_gpu, matchConf);
    ...
    if (save_graph)
    {
        ...
        f << matchesGraphAsString(img_names, pairwise_matches, confThresh);
    }

    // Leave only images we are sure are from the same panorama
    vector<int> indices = leaveBiggestComponent(features, pairwise_matches, confThresh);
    ...
    if (baCostFunc == "ray") adjuster = new detail::BundleAdjusterRay();
    else
    {
